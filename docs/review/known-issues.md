...
### 15. Beat System：拍间措辞复写 + stress 归零 —— 🆕 2026-08-22 真实 LLM 验收发现
- **现象**：同一事件内相邻文段拍的场景开头描写高度相似（paraphrase 级，bigram 相似度未达 0.6 阈值未被拦截）；情绪漂移叠加二次结算使 stress 快速衰减至 0。
- **修复方向**：①拍间相似度阈值下调至 ~0.45 或改逐句比较，beatSummaries 携带上一拍开头句供比对；②prompt 强化"禁止复用此前开头"；③stress/energy 等负向指标设漂移下限或纳入 #6 校准。
- **节奏固定"3+1"**：minGap=2 + mid 分支价值组合使节奏趋同，待 branchPotential 引导校准后应出现变节奏。
